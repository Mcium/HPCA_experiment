...
AcceleratorCore::DoAccelerate(int delay, int full_tile_size, double m, double n, double g, double power)
{
    ...
    //calculation_times = ceil(calculation_times);
    ...
    if (current_exec_time >= power_space || total_energy <= 0){
        // printf("per times: %f, current_exec_time:%f, total_energy:%f, power: %f, calculation_times: %f, delay:%d, total_cal_energy:%f, per energy: %f\n",
        //        delay * calculation_times, current_exec_time, total_energy, power, calculation_times, delay, total_cal_energy, power * calculation_times * delay / 1000000000);
        //        getchar();
        return 1;
    } else {
        return 0;
