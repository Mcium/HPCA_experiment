...
main()
{
    ...
    //input mehod
    std::cout << "input method(a, b, c):";
    std::cin >> method;
    std::cout << "current method is:" << method << std::endl;
    assert(method != "d" || method != "e");
    //input network name
    ...
    int tile_delay[layer_num];    //set delay
...
//  initialization done, start progress.==============================================================
    ...
    while(!Energy_trace.empty()){
        ...
        if (middle_m == M_data[middle_layer_num][energy_level] &&
            middle_g == G_data[middle_layer_num][energy_level] && energy_level != 0){
            ...
            for(int i = restart_layer; i < layer_num; i++){
                ...
                for(int j = restart_times; j < layer_times[i]; j++){
                    if (core->DoAccelerate(tile_delay[i], full_tile_size[i], M_data[i][energy_level], N_data[i][energy_level],
                        G_data[i][energy_level], Power_data[i][energy_level])){
    ...
    quit:
        ...
        for (int i = 0; i < layer_num; i++) {
            cal_rate += Power_data[i][energy_level]/(current_energy/(1000 * power_space));
        }
        if(energy_level != 0){
        ...
        // for (int i = 0; i < layer_num; i++) {
        //     std::cout << "," << M_data[i][energy_level] << "*" << N_data[i][energy_level]
